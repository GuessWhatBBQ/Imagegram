...
public class SessionService
{
    ...
    public SessionService(DbContextOptions<ApiContext> options)
    {
    ...
    {
        return await db.Sessions.Where(session => session.SessionToken == sessionToken).FirstOrDefaultAsync() ?? throw new SessionNotFoundException();
    }
    ...
    {
        var NewSession = await db.AddAsync(new Session { UserId = user.UserId, SessionToken = (Guid.NewGuid()).ToString() });
    ...
    {
        var Session = await db.Sessions.Where(session => session.SessionToken == sessionToken).FirstOrDefaultAsync() ?? throw new SessionNotFoundException();
    ...
    {
        var Sessions = await db.Sessions.Where(session => session.UserId == userId).ToListAsync() ?? throw new SessionNotFoundException();
